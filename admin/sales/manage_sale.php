...
<div class="card card-outline card-primary">
    ...
    <?php 
        $in_stock = $conn->query("SELECT SUM(quantity) as total FROM stock_list WHERE item_id = '{$k}' AND type = 1")->fetch_array()['total'];
        $out_stock = $conn->query("SELECT SUM(quantity) as total FROM stock_list WHERE item_id = '{$k}' AND type = 2")->fetch_array()['total'];
        $available_stock = $in_stock - $out_stock;
        
        $color = '';
        if($available_stock <= 0) {
            $status = '(Out of Stock)';
            $color = 'red';
        } elseif($available_stock < 50) {
            $status = '(Low Stock)';
            $color = 'red';
        } elseif($available_stock >= 50 && $available_stock <= 200) {
            $status = '(Medium Stock)';
            $color = 'yellow';
        } else {
            $status = '(In Stock)';
    ...
    ?>
    <option value="<?php echo $k ?>" <?php if($available_stock <= 0): ?>disabled<?php endif; ?> style="color: <?php echo $color ?>">
        <?php echo $v['name'] ?> <?php echo $status ?>
        <span style="display: inline-block; width: 10px; height: 10px; margin-left: 5px; background-color: <?php echo $color ?>;"></span>
    </option>
<?php endforeach; ?>
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <input type="number" step="any" class="form-control rounded-0" id="qty" min="1" required>
                            </div>
                        </div>
    ...
    <button id="save-button" class="btn btn-flat btn-primary" type="submit" form="sale-form" disabled>Save</button>
    <script>
  const remarksField = document.getElementById('remarks');
  const saveButton = document.getElementById('save-button');
  
  remarksField.addEventListener('input', () => {
    if (remarksField.value.trim().length > 0) {
