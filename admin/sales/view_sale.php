...
<button class="btn btn-primary" type="button" id="emailButton" data-bs-toggle="modal" data-bs-target="#emailModal"> <i class="fas fa-envelope"></i>
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" id="emailSubject" name="emailSubject" value="Sales Order Report" required>
    </div>
    <div class="mb-3">
        <label for="emailBody" class="form-label">Message</label>
        <textarea class="form-control" id="emailBody" name="emailBody" rows="3" required>Please Find The Attachment For The Sales Order Report</textarea>
    </div>
    ...
    <i class="fas fa-envelope"></i> Send Email
        ...
        <a class="btn btn-flat btn-info" href="<?php echo base_url.'/admin?page=sales/manage_sale&id='.(isset($id) ? $id : '') ?>">
        <i class="fas fa-edit"></i> Edit</a>
        <a class="btn btn-flat btn-dark" href="<?php echo base_url.'/admin?page=sales' ?>"> <i class="fas fa-list"></i> Back To List</a>
    </div>
</div>
...
<script>
    ...
    $('#pdf').click(function(){
        ...
        _el.append('<div class="d-flex justify-content-center">'+
                  ...
                  '<h3 class="text-center">Sale Order</h3>'+
                  '</div>'+
                  '<div class="col-1 text-right">'+
                  '</div>'+
                  '</div><hr/>')
...
$(function(){
    $('#excel').click(function(){
        ...
        var _head = $('head').clone()
        _head.find('title').text("Sale Order Details - Excel Export")
        var header = $('<div>')
        header.append(_head)
        ...
        var ctx = {worksheet: 'Sale Order', table: html}
        var link = document.createElement("a")
        link.download = "sale_order.xls"
...
$(document).ready(function() {
    ...
    $('#emailForm').submit(function(e) {
        ...
        html2pdf().set(opt).from(contentHtml).toPdf().get('pdf').then(function(pdf) {
            ...
            formData.append('pdf', pdfOutput, 'Sales_Order_Detail.pdf');
            ...
            // Send data via AJAX
            $.ajax({
                url: '/sms/admin/sales/send_mail.php',
                method: 'POST',
                data: formData,
                processData: false,
                contentType: false,
