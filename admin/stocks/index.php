<div class="card card-outline card-primary">
    ...
    <div class="container-fluid">
                ...
                <tbody>
                    <?php 
                        ...
                        while($row = $qry->fetch_assoc()):
                            ...
                            // Throw an error if available stocks is 0 or negative
                            if($row['available'] <= 0) {
                                echo '<script>alert("'.$row['name'].' is out of stock.");</script>';
                                continue; // Skip this item and continue with the next one
                            }

                            // Set the stock level box color based on the available quantity
                            if ($row['available'] < 50) {
                                $box_color = 'red';
                            } elseif ($row['available'] >= 50 && $row['available'] <= 200) {
                                $box_color = 'yellow';
                            } else {
                                $box_color = 'green';
                            }
        ...
        } elseif ($available >= 50 && $available <= 200) {
            $box_color = 'yellow';
            $message = 'Stock is Medium.';
        } else {
            $box_color = 'green';
            $message = 'Stock is good!';
        }
    ?>
	<div style="background-color: <?php echo $box_color; ?>;" title="<?php echo $message; ?>">
