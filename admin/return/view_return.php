...
<button class="btn btn-primary" type="button" id="emailButton" data-bs-toggle="modal" data-bs-target="#emailModal"> <i class="fas fa-envelope"></i>
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" id="emailSubject" name="emailSubject" value="Return Order Report" required>
    </div>
    <div class="mb-3">
        <label for="emailBody" class="form-label">Message</label>
        <textarea class="form-control" id="emailBody" name="emailBody" rows="3" required>Please Find The Attachment For The Return Order Report</textarea>
    </div>
...
$(document).ready(function() {
    ...
    $('#emailForm').submit(function(e) {
        ...
        _el.append('<div class="d-flex justify-content-center">'+
                  ...
                  '<div class="col-10">'+
                  '<h2 class="text-center"><?php echo $_settings->info('name') ?></h2>'+
                  '<h3 class="text-center">Return Order Details</h3>'+
                  '</div>'+
                  '<div class="col-1 text-right">'+
                  '</div>'+
                  '</div><hr/>');
        ...
        html2pdf().set(opt).from(contentHtml).toPdf().get('pdf').then(function(pdf) {
            ...
            formData.append('pdf', pdfOutput, 'Return_Order_Detail.pdf');
            ...
            // Send data via AJAX
            $.ajax({
                url: '/sms/admin/return/send_mail.php',
                method: 'POST',
                data: formData,
                processData: false,
                contentType: false,
