...
<div class="card card-outline card-primary">
    ...
    <button class="btn btn-flat btn-secondary" type="button" id="pdf">
...
<button class="btn btn-primary" type="button" id="emailButton" data-bs-toggle="modal" data-bs-target="#emailModal"> <i class="fas fa-envelope"></i>
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" id="emailSubject" name="emailSubject" value="Received Order Details Report" required>
    </div>
    <div class="mb-3">
        <label for="emailBody" class="form-label">Message</label>
        <textarea class="form-control" id="emailBody" name="emailBody" rows="3" required>Please Find The Attachment For The Received Order Detao; Report</textarea>
    </div>
...
<script>
    ...
    $('#pdf').click(function(){
        ...
        _el.append('<div class="d-flex justify-content-center">'+
                  ...
                  '<h3 class="text-center">Received Order</h3>'+
                  '</div>'+
                  '<div class="col-1 text-right">'+
                  '</div>'+
                  '</div><hr/>')
...
$(function(){
    $('#excel').click(function(){
        ...
        var _head = $('head').clone()
        _head.find('title').text("Received Order Details - Excel Export")
        var header = $('<div>')
        header.append(_head)
        ...
        var ctx = {worksheet: 'Received Order', table: html}
        var link = document.createElement("a")
        link.download = "received_order.xls"
...
$(document).ready(function() {
    ...
    $('#emailForm').submit(function(e) {
        ...
        _el.append('<div class="d-flex justify-content-center">'+
                  ...
                  '<div class="col-10">'+
                  '<h2 class="text-center"><?php echo $_settings->info('name') ?></h2>'+
                  '<h3 class="text-center">Received Order Details</h3>'+
                  '</div>'+
                  '<div class="col-1 text-right">'+
                  '</div>'+
                  '</div><hr/>');
        ...
        var opt = {
            ...
            filename: 'Received_Order_Detail.pdf',
            image: { type: 'jpeg', quality: 0.98 },
            html2canvas: { scale: 2 },
            jsPDF: { unit: 'in', format: 'letter', orientation: 'portrait' }
        };
        ...
        html2pdf().set(opt).from(contentHtml).toPdf().get('pdf').then(function(pdf) {
            ...
            formData.append('pdf', pdfOutput, 'Received_Order_Detail.pdf');
            ...
            // Send data via AJAX
            $.ajax({
                url: '/sms/admin/receiving/send_mail.php',
                method: 'POST',
                data: formData,
                processData: false,
                contentType: false,
