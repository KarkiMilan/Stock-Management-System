...
$result_this_month = $conn->query("SELECT DATE(date_created) as sales_date, SUM(amount) as total_sales 
FROM `sales_list` 
...
$result_this_year = $conn->query("SELECT DATE_FORMAT(date_created, '%Y-%m') as sales_date, SUM(amount) as total_sales 
FROM `sales_list` 
WHERE YEAR(date_created) = YEAR(CURRENT_DATE()) 
...
$sales_data_this_year[] = $row;
            ...
            $result_all_time = $conn->query("SELECT DATE(date_created) as sales_date, SUM(amount) as total_sales 
            FROM `sales_list` 
            GROUP BY DATE(date_created) 
